...
class ResistiveVoltageQWidget(SettingsMPLQWidget):
    ...
    def get_df_resistive_voltage(self):
        ...
        df['Units'].loc[df['time'] < self.idot_peak_time] = 0
        return df
    ...
    def save_origin_pro(self, op):
        ...
        try:
            ...
            self.parent.graph_c_v_r[2].set_xlim(begin=0, end=self.df_resistive_voltage['time'].max())
            self.parent.graph_c_v_r[2].set_ylim(begin=0, end=self.df_resistive_voltage['Units'].max())
