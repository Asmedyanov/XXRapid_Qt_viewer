...
from XXRapid_Qt_viewer.ExperimentQWidget.Waveform.WaveformProcessingQTabWidget.WaveformPhysicalValuesQWidget.WaveformIdotQWidget.WaveformIdotSettingsQWidget import *
...
class WaveformIdotQWidget(SettingsMPLQWidget):
    ...
    def __init__(self, df_current, settings_dict=None, timeshift=0):
        ...
        self.IdotPlot, = self.MPLQWidget.ax.plot(self.df_idot_to_plot['time'] * 1e6,
                                                 self.df_idot_to_plot['Units'] * 1e-12, label='original')
        self.IdotPlot_smoothed, = self.MPLQWidget.ax.plot(self.df_idot_smoothed_to_plot['time'] * 1e6,
    ...
    def on_settings_box(self):
        ...
        self.df_idot_smoothed = self.get_df_idot_smoothed()
        self.df_idot_smoothed_to_plot = self.get_idot_smoothed_to_plot()
        self.IdotPlot_smoothed.set_data(self.df_idot_smoothed_to_plot['time'] * 1e6,
                               self.df_idot_smoothed_to_plot['Units'] * 1e-12)
    ...
    def set_data(self, df_current, timeshift=0):
        ...
        self.df_idot_smoothed = self.get_df_idot_smoothed()
        self.df_idot_smoothed_to_plot = self.get_idot_smoothed_to_plot()
        self.IdotPlot.set_data(self.df_idot_to_plot['time'] * 1e6,
